...
This was a real wolf in sheeps clothing. It comes across in a relatively simple way, but exploring each point reveals more and more unanswered questions. The Tickerboard itself has a whole host of potential features involved, and is ripe for feature creep if we started out with such a bare-bones requirement.
...
Overall I know I am leaning towards my area of knowledge, and as noted below, a lot of these would be heavily influenced by our existing infrastructure and what kind of people and expertise is available for the job.
...
1. **Airline Interface App**
    ...
    This could be an enterprise-enabled applciation or even command line that allowed them to query, add, or make updates to schedules. There is a lot to unpack from the requirements around this one, more on that below - this is my least comfortable recommendation by far.
...
- [x] The flight information needs to be viewable over the internet.
    - This is an interesting requirement, and very ambiguous - will we be providing this interface? What requirements are needed for querying the data? Is there any requirement for a user identity before sharing flight information?
        ...
        I am going to operate under the ***(big)*** assumption that a given Airline, or other third party (such as a travel agency, google plugin, or the like) will be interfacing with our API, and we will NOT be providing an end-user interface for this feature. It's not the end-of-the-world if we must, but it would likely require more people involved and some minor infrastructure tweaks (CDN placement etc...) to meet that need.
...
This is one place I struggle with - it's hard to predict how much time you can really devote to a project. The work doesn't happen in a bubble. I would be seeking a lot of advice here before having a lot of confidence in my results.

### Bare Mininmums:

With some lead time to design and plan, perhaps get some extra eyes on the process, I am confident that an MVP could be completely tested and available within 6 months or less of the epics being finalized and the above requirements / debates being resolved. I think a team of 5-7 could handle the workload, assuming we had the skills needed. The first thing would be modelling the database, and providing test data for the other workstreams to take advantage of.
...
This is assuming appropriate expertise and availability of team members, a team of 7 variously skilled engineers could handle something like this. To be honest, many choices are malleable here, and even some relatively innocous changes would shift the deadline, especially given the abilities of the team if we were under a tight deadline.
...
- A basic react app would mean there is a lot of expertise available for maintance - plenty of people know react. We would want to balance this when picking a stack - is it trending? Who knows it? While the app doesn't have a great deal of complexity, we would be wise to keep it up to date.
...
- This could be an enterprise-enabled applciation or even command line that allowed them to query, add, or make updates to schedules. The requirement of having no *interface* accessible on the internet would be met, but it still seems as though there is more to unpack from that requirement, and this approach is not as simple from a maintainence and support perspective as a cloud-based application from a support perspective.


# Closing

That was a very interesting process, and I feel as though I am arbitrarily stopping my effort here - there are plenty of unaswered questions and unexplored technologies, but I am hoping to have the opportunity to dicuss and see how other people approached the same problem.

Thanks for reading through all of this soup, and making it this far. I really appreciate your consideration,
